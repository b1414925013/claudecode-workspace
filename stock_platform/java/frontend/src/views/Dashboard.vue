...
onMounted(async () => {
  try {
    ...
    const tradeResult = await tradeApi.list({ page: 1, perPage: 5 })
    recentTrades.value = tradeResult.items
    const posResult = await positionApi.list({ page: 1, perPage: 100 })
    positions.value = posResult.items
  } catch (e) {
